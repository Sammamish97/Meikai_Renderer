...
CalcIBLDiffusePass::CalcIBLDiffusePass(DXApp* appPtr, ComPtr<ID3DBlob> computeShader, UINT hdrCubeSrvIdx,
	UINT IBLDiffuseCubeUrvIdx)
		:IPass(appPtr, computeShader)
{
	InitRootSignature();
	InitPSO();
	mDiffuseDescIndices.HDR_SRV_CUBE = hdrCubeSrvIdx;
	mDiffuseDescIndices.UAV_IBL_DIFFUSE_CUBE = IBLDiffuseCubeUrvIdx;
}

void CalcIBLDiffusePass::InitRootSignature()
{
    ...
    rootParameters[3].InitAsConstants(mDiffuseDescIndices.TexNum + 1, 1);
...
void CalcIBLDiffusePass::InitPSO()
{
    auto device = mApp->GetDevice();
